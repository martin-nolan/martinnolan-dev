...
export default function WritingPreview({ items }: WritingPreviewProps) {
  return (
    <section id="principles" aria-labelledby="principles-title" className="mx-auto max-w-6xl px-5 py-20 sm:px-8 sm:py-24">
      <Reveal>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {items.map((item, index) => (
          <Reveal key={item.title} delayMs={index * 90}>
            <article className="flex h-full flex-col rounded-[1.8rem] border border-[color:var(--border-soft)] bg-[color:var(--paper)] p-6">
              ...
              <p className="mt-3 grow text-sm leading-7 text-[color:var(--ink-muted)]">
                {item.summary}
              </p>
            </article>
          </Reveal>
