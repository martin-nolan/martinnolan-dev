        ...
        <div className="grid gap-4 lg:grid-cols-3">
          {items.map((item, index) => (
            <Reveal key={item.label} delayMs={index * 100}>
              <article className="rounded-[1.8rem] border border-[color:var(--border-soft)] bg-[color:var(--paper)] p-5 shadow-[0_20px_50px_rgba(78,65,52,0.05)]">
                <p className="text-[1.8rem] font-semibold tracking-[-0.04em] text-[color:var(--ink-strong)]">
                  {item.value}
                </p>
